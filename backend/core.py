...
class Core:
    ...
    def generate_schema(self, table_description : str, table_rules : str, existed_tables_str : str) -> str :
        """
            Generate schema for a table
        """
        logger.info("Generate_schema...")
        existed_tables = existed_tables_str.split()
        table_schema, table_name, tokens_used = self.llm_backend.generate_schema(table_description, table_rules, existed_tables)
    ...
    def generate_sql(self, table_schema : str, script_definition : str, existed_tables_str : str) -> str:
        ...
        existed_tables = existed_tables_str.split()
        new_tables, table_sql, local_errors, tokens_used = self.llm_backend.generate_sql(table_schema, script_definition, existed_tables)
